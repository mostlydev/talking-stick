...
- `--interrupt` forces one native submission per message, even with a live listener or an earlier unread wake. For human senders, Claude Code receives `priority: now`, requesting cancellation of its active turn before reading the urgent prompt. Agent senders inject at the next tool boundary without requesting cancellation. Its inbound controls still apply. Codex queues the prompt; its queue CLI cannot cancel an active turn. Results report `interrupt_status: requested` for human-to-Claude cancellation requests, `injected` for agent-to-Claude urgent prompts, or `unsupported` for transports without immediate cancellation. Pending urgent deliveries expire after 60 seconds rather than interrupting unrelated later work; the room message remains readable. These statuses describe the request, not proof that the harness acted on it.
