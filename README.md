...
After you send a directed message, the live panel shows a compact status per recipient for your latest send. It updates from `queued` to `delivered` when the agent's receiver returns your message, without adding status lines to chat history. Unavailable or unconfirmed wake transports remain explicit. This is a delivery receipt, not proof the model has acted.
