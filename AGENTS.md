...
- `~/.claude/skills/talking-stick` and `~/.grok/skills/talking-stick` may be symlinked back to `skills/talking-stick` via `npm link` + `tt install --link`; edits via either path propagate. Be intentional about which file you're opening.
- CLI identity first checks harness signals such as `CLAUDECODE`, `CODEX_THREAD_ID`, `GEMINI_CLI`, `CMUX_AGENT_LAUNCH_KIND=grok`, `OPENCODE`, or explicit `TT_HARNESS_AGENT_ID=<id>`. It falls back to `human:<user>` only when no harness signal is present. `TT_HARNESS_EXPORT=1` enables ancestry-only detection for most harnesses when env markers are unavailable. Grok does not depend on cmux: without a Grok env marker, `tt` still walks process ancestry for a `grok` root process. Grok's `GROK_SESSION_ID` is hook-only; `tt install grok` installs `~/.grok/hooks/talking-stick-session.json`, and that hook records session context in `${TALKING_STICK_DATA_DIR}/grok-sessions.jsonl` for later identity upgrades. MCP path always derives harness identity from env/ancestry. Use `tt whoami --explain` to see the decision.
