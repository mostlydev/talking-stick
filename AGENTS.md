...
- CLI identity first checks harness signals such as `CLAUDECODE`, `CODEX_THREAD_ID`, `GEMINI_CLI`, `OPENCODE`, or explicit `TT_HARNESS_AGENT_ID=<id>`. It falls back to `human:<user>` only when no harness signal is present. `TT_HARNESS_EXPORT=1` enables ancestry-only harness detection when env markers are unavailable. MCP path always derives harness identity from env/ancestry. Use `tt whoami --explain` to see the decision.
