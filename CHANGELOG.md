...
- The shipped skill and default instructions no longer treat being asked to use Talking Stick as a
  task in itself. An agent invoked with no work is told to join the room and go idle in
  `tt standby`, waiting for the task to arrive in `tt chat`, rather than claiming the stick or asking
  for direction in a harness prompt the operator is not reading. A session that cannot self-wake
  keeps exactly one `tt wait --park` so it stays reachable without being granted a turn it has no
  work for. An explicit task given alongside the skill still runs the normal loop.
- Agents are instructed not to relay operator messages to each other. An operator's room message
  already reaches every joined agent, and a directed message was scoped deliberately, so
  re-broadcasting either duplicated the console or widened a scope the operator chose. Agents still
  report their own actions, findings, and disagreements.
- Agents are instructed not to announce their arrival in the room. The join event is already
  rendered in the operator console, so the extra message was a duplicate line; agents introduce
  themselves when the operator asks rather than on launch.
